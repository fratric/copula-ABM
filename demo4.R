...
MsimXJoe <- matrix(nrow = sim, ncol = Tt)
deltaPsimXJoe <- matrix(nrow = sim, ncol = Tt)
...
registerDoParallel()
listFinal <- foreach(s=1:sim) %dopar% {
  x <- simulation(a = a, sigmadelta = sigmadelta, sigmaalfa = sigmaalfa, sigmabeta = sigmabeta, meanf = meanf, 
                  sdf = sdf, meancc = meancc, sdcc = sdcc, pt1 = 10, pt2 = 13, joeParam = 8)
}

listFinalJoe <- foreach(s=1:sim) %dopar% {
  x <- simulation(a = a, sigmadelta = sigmadelta, sigmaalfa = sigmaalfa, sigmabeta = sigmabeta, meanf = meanf, 
                  sdf = sdf, meancc = meancc, sdcc = sdcc, pt1 = 10, pt2 = 8.5, joeParam = 8)
}
stopCluster(cl)


for(s in 1:sim){
  deltaPsimX[s,] <- listFinal[[s]]$deltap
  MsimX[s,] <- listFinal[[s]]$pt
  
  deltaPsimXJoe[s,] <- listFinalJoe[[s]]$deltap
  MsimXJoe[s,] <- listFinalJoe[[s]]$pt
}

hist(deltaPsimX[,3], breaks = 20, main = "", xlab = "increment", probability = TRUE)
lines(density(deltaPsimX[,3]),col="red")
hist(deltaPsimXJoe[,3], breaks = 20, main="", xlab = "increment", probability = TRUE)
lines(density(deltaPsimXJoe[,3]),col="red")

matplot(t(deltaPsimX[,2:3]), type="l", ylab = "delta p", main = "", xlab = "time", xaxt="n")
axis(1, at = c(1,2))
matplot(t(deltaPsimXJoe[,2:3]), type="l", ylab = "delta p", main = "", xlab = "time", xaxt="n")
axis(1, at = c(1,2))
